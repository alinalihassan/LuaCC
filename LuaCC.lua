...
local FileExtension = ".luacc"
local CaseSensitive = true
local Minify = false
local IncrementaryOperators = true
local AssignmentOperators = true
local TypeClasses = true
local StrictClasses = true
local Conditional = true
local UsingNamespace = true
local TypeOperators = true
local BuiltIn = [[]]
...
LuaCC = {
	Compile = function(file,name )
		...
		local x = io.open(file)
		code = x:read("*all")
		x:close()
		local filename = name or "Compiled "..file
